...
export class StandardSokobanAnnotationTokennizer {
    translate(encodedLevel) {
        const lines = this.splitInLines(replaceImplicitLayeredTiles(encodedLevel.toLowerCase()));
    ...
    createRectangularLayeredMatrix(height, width, baseMatrix) {
        return new Array(height)
            .fill(new Array(width)
            .fill(null))
            .map((line, y) => line
            .map((_, x) => (baseMatrix[y][x] || [])
            .map(tile => ({
            code: tile.code,
            orientation: tile.orientation
        }))));
    }
    splitInLines(encodedLevel) {
        return encodedLevel
            .split(/[\n|]/);
    }
    removeMetaChars(metamap) {
        ...
        // #
        return metamap
            .map((line, index) => (line.match(tileRegex) || [])
    ...
    getOrientedTilesFromExpression(expression) {
        // 14lt => 14, l, t
